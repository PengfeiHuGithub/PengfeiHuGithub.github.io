...
## 一、基础：Vue.js 组件的三个 API：prop、event、slot
...
- [Vue 组件通信之 Bus(opens new window)](https://juejin.im/post/5a4353766fb9a044fb080927)
- [Vuex 通俗版教程(opens new window)](https://juejin.im/entry/58cb4c36b123db00532076a2)
...
> [https://cn.vuejs.org/v2/api/#provide-inject(opens new window)](https://cn.vuejs.org/v2/api/#provide-inject)
>
...
> 可以看到，在 A.vue 里，我们设置了一个 **provide: name**，值为 Aresn，它的作用就是将 **name** 这个变量提供给它的所有子组件。而在 B.vue 中，通过 `inject` 注入了从 A 组件中提供的 **name** 变量，那么在组件 B 中，就可以直接通过 **[this.name (opens new window)](http://this.name/)**访问这个变量了，它的值也是 Aresn。这就是 provide / inject API 最核心的用法。

需要注意的是：

> provide 和 inject 绑定并**不是可响应**的。这是刻意为之的。然而，如果你传入了一个可监听的对象，那么其对象的属性还是可响应的。

所以，上面 A.vue 的 name 如果改变了，B.vue 的 [this.name (opens new window)](http://this.name/)是不会改变的，仍然是 Aresn。
...
> 上面，我们把整个 app.vue 的实例 `this` 对外提供，命名为 **app**（这个名字可以自定义，推荐使用 app，使用这个名字后，子组件不能再使用它作为局部属性）。接下来，任何组件（或路由）只要通过 `inject` 注入 app.vue 的 app 的话，都可以直接通过 **[this.app.xxx (opens new window)](http://this.app.xxx/)**来访问 app.vue 的 `data`、`computed`、`methods` 等内容。
...
> 如果这是你第一次听说 `provide / inject` 这对 API，一定觉得它太神奇了，至少笔者第一时间知晓时是这样的。它解决了独立组件间通信的问题，用好了还有出乎意料的效果。笔者在开发 [iView Developer (opens new window)](https://dev.iviewui.com/)时，全站就是使用这种方法来做全局数据的管理的，如果你有机会一个人做一个项目时，不妨试试这种方法。

> 下一节，将介绍另一种组件间通信的方法，不同于 `provide / inject` 的是，它们不是 Vue.js 内置的 API。

## 三、组件的通信 2：派发与广播——自行实现 dispatch 和 broadcast 方法
...
（也可通过在线链接 [https://run.iviewui.com/ggsomfHM (opens new window)](https://run.iviewui.com/ggsomfHM)直接运行该示例）
...
注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/blob/2.0/src/mixins/emitter.js)。

## 四、组件的通信 3：找到任意组件实例——findComponents 系列方法
...
>
> 完整源码地址：[https://github.com/icarusion/vue-component-book(opens new window)](https://github.com/icarusion/vue-component-book)
...
- [ES6 Module 的语法(opens new window)](http://es6.ruanyifeng.com/#docs/module)

注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/blob/2.0/src/utils/assist.js)。

## 五、实战 1：具有数据校验功能的表单组件——Form
...
Form 要用到数据校验，并在对应的 FormItem 中给出校验失败的提示，校验我们会用到一个开源库：[async-validator (opens new window)](https://github.com/yiminghe/async-validator)，基本主流的组件库都是基于它做的校验。使用它很简单，只需按要求写好一个校验规则就好，比如：
...
> 本节所有代码可以在 [https://github.com/icarusion/vue-component-book (opens new window)](https://github.com/icarusion/vue-component-book)中查看，你可以一边看源码，一边阅读本节；也可以边阅读，边动手实现一遍，遇到问题再参考完整的源码。
...
[https://github.com/icarusion/vue-component-book(opens new window)](https://github.com/icarusion/vue-component-book)
...
注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/tree/2.0/src/components/form)。

### 扩展阅读

- [一份超级详细的Vue-cli3.0使用教程(opens new window)](https://juejin.im/post/5bdec6e8e51d4505327a8952)

## 六、实战 2：组合多选框组件——CheckboxGroup & Checkbox
...
> 友情提示：请先阅读 Vue.js 文档的 [https://cn.vuejs.org/v2/guide/forms.html#复选框 (opens new window)](https://cn.vuejs.org/v2/guide/forms.html#复选框)内容。
...
Checkbox 新增的 prop： `label` 只会在组合使用时有效，结合 `model` 来使用，用法已在 Vue.js 文档中介绍了 [https://cn.vuejs.org/v2/guide/forms.html#复选框 (opens new window)](https://cn.vuejs.org/v2/guide/forms.html#复选框)。
...
注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/tree/2.0/src/components/checkbox)。

## 七、Vue 的构造器——extend 与手动挂载——$mount
...
使用 Vue.js 也有二八原则，即 80% 的人看过 [Vue.js 文档教程篇 (opens new window)](https://cn.vuejs.org/v2/guide/)，20% 的人看过 [Vue.js 文档 API (opens new window)](https://cn.vuejs.org/v2/api/)。

下一节，我们来做点有趣的东西。

### 扩展阅读

- [聊聊 Vue.js 的 template 编译(opens new window)](https://juejin.im/post/59da1c116fb9a00a4a4cf6dd)

## 八、实战 3：动态渲染 .vue 文件的组件—— Display



你可能用过 [jsfiddle (opens new window)](https://jsfiddle.net/)或 [jsbin (opens new window)](https://jsbin.com/)之类的网站，在里面你可以用 CDN 的形式引入 Vue.js，然后在线写示例，实时运行，比如下面这个例子：

[https://jsfiddle.net/c87yh92v/(opens new window)](https://jsfiddle.net/c87yh92v/)

不过，这类网站主要是一个 html，里面包含 js、css 部分，渲染侧是用 iframe 嵌入你编写的 html，并实时更新。在这些网站写示例，是不能直接写 `.vue` 文件的，因为没法进行编译。

再来看另一个网站 [iView Run (opens new window)](https://run.iviewui.com/)（之前小节也有提到），它是能够在线编写一个标准的 `.vue` 文件，并及时渲染的，它也预置了 iView 环境，你可以使用 iView 组件库全部的组件。本小节，我们就来实现这样一个能够动态渲染 .vue 文件的 `Display` 组件，当然，用到的核心技术就是上一节的 `extend` 和 `$mount`。
...
- [new Function(opens new window)](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Function)
- [Vue.js 2.0 独立构建和运行时构建的区别(opens new window)](https://jingsam.github.io/2016/10/23/standalone-vs-runtime-only-build-in-vuejs2.html)
...
注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/tree/2.0/src/components/base/notification)。

## 十、更灵活的组件：Render 函数与 Functional Render
...
1. 对应属性的数据对象，比如组件的 `props`、元素的 `class`、绑定的事件、`slot`、自定义指令等，该参数是可选的，上文所说的 Render 配置项多，指的就是这个参数。该参数的完整配置和示例，可以到 Vue.js 的文档查看，没必要全部记住，用到时查阅就好：[createElement 参数 (opens new window)](https://cn.vuejs.org/v2/guide/render-function.html#createElement-参数)。
2. 子节点，可选，`String` 或 `Array`，它同样是一个 `h`。示例：
...
> Render 函数的基本用法还有很多，比如 v-model 的用法、事件和修饰符、slot 等，读者可以到 Vue.js 文档阅读。[Vue.js 渲染函数(opens new window)](https://cn.vuejs.org/v2/guide/render-function.html)
...
// 正确写法 import Alert from './alert.vue'; import Vue from 'vue';

Alert.newInstance = properties => { const props = properties || {};

const Instance = new Vue({ data: props, render (h) { return h(Alert, { props: props }); } });

const component = Instance.$mount(); document.body.appendChild(component.$el);

const alert = Instance.$children[0];

return { add (noticeProps) { alert.add(noticeProps); }, remove (name) { alert.remove(name); } } }; export default Alert;
...
> 您可以阅读 [Vue.js 文档—函数式组件 (opens new window)](https://cn.vuejs.org/v2/guide/render-function.html#函数式组件)来查看示例。
...
注：本节部分内容参考了《Vue.js 实战》（清华大学出版社），部分代码参考 [iView (opens new window)](https://github.com/iview/iview/blob/2.0/src/components/transfer/transfer.vue)。

## 十一、实战 5：可用 Render 自定义列的表格组件——Table



> 表格组件 Table 是中后台产品中最常用的组件之一，用于展示大量结构化的数据。大多数组件库都提供了表格组件，比如 [iView (opens new window)](https://www.iviewui.com/components/table)，功能也是非常强大。正规的表格，是由 `<table>`、`<thead>`、`<tbody>`、`<tr>`、`<th>`、`<td>` 这些标签组成，一般分为表头 **columns** 和数据 **data**。本小节就来开发一个最基本的表格组件 Table，它支持使用 Render 函数来自定义某一列。
...
> 完整的代码见：[https://github.com/icarusion/vue-component-book/blob/master/src/views/table-render.vue(opens new window)](https://github.com/icarusion/vue-component-book/blob/master/src/views/table-render.vue)
...
- [Div 和 Table 的区别(opens new window)](https://www.cnblogs.com/lovebear/archive/2012/04/18/2456081.html)

## 十二、实战 6：可用 slot-scope 自定义列的表格组件——Table
...
- [异步组件(opens new window)](https://cn.vuejs.org/v2/guide/components-dynamic-async.html#异步组件)
...
> 意，这里的 `v-if="data.expand"` 并不是递归组件的终结条件，虽然它看起来像是一个可以为 `false` 的判断语句，但它的用处是判断当前节点**的子节点**是否展开（渲染），如果当前节点不展开，那它所有的子节点也就不会展开（渲染）。
...
以上就是 Tree 组件的所有内容，完整的代码见： [https://github.com/icarusion/vue-component-book/tree/master/src/components/tree(opens new window)](https://github.com/icarusion/vue-component-book/tree/master/src/components/tree)

用例：[https://github.com/icarusion/vue-component-book/blob/master/src/views/tree.vue(opens new window)](https://github.com/icarusion/vue-component-book/blob/master/src/views/tree.vue)
...
- [浅拷贝与深拷贝(opens new window)](https://juejin.im/post/5b5dcf8351882519790c9a2e)

注：本节部分代码参考 [iView (opens new window)](https://github.com/iview/iview/blob/2.0/src/utils/assist.js#L114)。

## 十五、拓展：Vue.js 容易忽略的 API 详解



前面的小节已经覆盖了 Vue.js 组件的绝大部分内容，但还是有一些 API 容易忽略。本节则对 Vue.js 的一些重要且易忽略的 API 进行详细介绍。
...
nextTick 的源码在 [https://github.com/vuejs/vue/blob/dev/src/core/util/next-tick.js (opens new window)](https://github.com/vuejs/vue/blob/dev/src/core/util/next-tick.js)，可以看到，Vue.js 使用了 `Promise`、`setTimeout` 和 `setImmediate` 三种方法来实现 nextTick，在不同环境会使用不同的方法。
...
> [delimiters(opens new window)](https://cn.vuejs.org/v2/api/#delimiters)

改变纯文本插入分隔符，Vue.js 默认的是 ``，如果你使用其它一些后端模板，比如 Python 的 Tornado 框架，那 Vue.js 和 Tornado 的 ``就冲突了，这时用它可以修改为指定的分隔符。

> [v-once(opens new window)](https://cn.vuejs.org/v2/api/#v-once)

只渲染元素和组件**一次**。随后的重新渲染，元素/组件及其所有的子节点将被视为静态内容并跳过。这可以用于优化更新性能。

> [vm.$isServer(opens new window)](https://cn.vuejs.org/v2/api/#vm-isServer)

当前 Vue 实例是否运行于服务器，如果你的组件要兼容 SSR，它会很有用。

> [inheritAttrs(opens new window)](https://cn.vuejs.org/v2/api/#inheritAttrs)

一些原生的 html 特性，比如 `id`，即使没有定义 props，也会被集成到组件根节点上，设置 inheritAttrs 为 false 可以关闭此特性。

> [errorHandler(opens new window)](https://cn.vuejs.org/v2/api/#errorHandler)

使用 `errorHandler` 可以进行异常信息的获取。

> [watch(opens new window)](https://cn.vuejs.org/v2/api/#watch)
...
> [comments(opens new window)](https://cn.vuejs.org/v2/api/#comments)

开启会保留 html 注释。

> [transition(opens new window)](https://cn.vuejs.org/v2/api/#transition)

内置的组件，可做过渡效果，比如 CSS 的高度从 0 到 auto（使用纯 CSS 是无法实现动画的）。

### 结语

彻底掌握一门语言（框架），不需要阅读它所有的源码，但至少要阅读它所有的 [API (opens new window)](https://cn.vuejs.org/v2/api/)。

## 十六、拓展：Vue.js 面试、常见问题答疑
...
这是个概念题，主要考察候选人是否知道这个用法。简单说，就是把一个组件的编译缓存起来。在第 14 节有过详细介绍，也可以看看 [Vue.js 的文档 (opens new window)](https://cn.vuejs.org/v2/guide/components-dynamic-async.html#在动态组件上使用-keep-alive)。
...
[Vue.js 生命周期 (opens new window)](https://cn.vuejs.org/v2/api/#选项-生命周期钩子)主要有 8 个阶段：
...
Vue.js 文档已经对 [深入响应式原理 (opens new window)](https://cn.vuejs.org/v2/guide/reactivity.html)解释的很透彻了。
