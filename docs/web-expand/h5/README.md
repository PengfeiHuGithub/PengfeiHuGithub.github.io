...
在腾讯，「H5 开发」有设立对应的实体岗位：「[UI 开发工程师 (opens new window)](https://www.lagou.com/jobs/2634151.html)」，这个岗位早在两三年前由「网页重构工程师」演变而来，最早出现在 SNG（社交网络事业群）的用户体验设计部（[ISUX (opens new window)](https://isux.tencent.com/)）。与阿里和百度不同的是，腾讯的岗位职能分得比较细，传统的「网页重构工程师」虽然也属于前端范畴，但其工作职责主要负责静态网页制作（设计稿还原成为网页）和少量的 JavaScript 脚本逻辑开发，但随着 HTML5 和 CSS3 相关技术标准的出现与普及，重构工程师除了前面提及的基本工作，还要肩负「CSS3 动效开发」等 UI 相关的工作，「重构」的工作定义已然无法契合新时代的要求，于是便有了「H5 开发」的概念。

在京东，前端开发岗目前尽管没有细分出「H5 开发工程师」或「UI 开发工程师」这种实体的职位，但为了针对性地招聘人才，一些技术部门（如凹凸实验室）仍然会以「[H5 开发工程师 (opens new window)](https://aotu.io/join/)」或「[H5 前端开发工程师 (opens new window)](https://aotu.io/join/)」来招人。「H5 开发工程师」的职责要求其实与腾讯的「UI 开发工程师」基本一样，未来也许会统一沿用腾讯「UI 开发工程师」的叫法并设立相应的实体职位。「H5 开发」其实不太需要后端开发经验（有则为加分项），偏向界面还原制作、前端脚本逻辑的实现，同时与谷歌 2017 年提出的新岗位-动效设计师（[Motion Designer (opens new window)](https://design.google/jobs/motion-designer/)）也有交集，要求具备动效设计以及开发的能力，一句话来概括就是「基于 HTML5、CSS3 等网页技术，负责可视化 UI 界面及动效的开发」。
...
一名合格的「H5 开发工程师」不仅需要会做「PC 端网页」、「移动端网页」，还需要会做各类强交互、多动效的「 [HTML5 营销活动页面 (opens new window)](https://cases.aotu.io/)」，甚至还要做动效及脚本逻辑复杂的「[HTML5 小游戏 (opens new window)](https://cases.aotu.io/cates/游戏型/index.html)」。
...
这个能力模型长着一张三角脸，往下代表能力的基础性，往上代表能力的进阶。其中「基础页面开发」、「响应式页面开发」、「滑屏应用开发」以及「动画效果开发」是岗位的基础能力要求，「游戏开发」是高阶能力要求。换一个说法，掌握「基础页面开发」，我们就能应付「PC 端网页」的开发；掌握「响应式页面开发」，我们可以撸「移动端网页」；掌握了「滑屏应用开发」以及「动画效果开发」，我们能开发各类强交互、多动效的「 [HTML5 营销活动页面 (opens new window)](https://cases.aotu.io/)」；而掌握了「H5 游戏开发」，我们才能做动效及脚本逻辑复杂的「[HTML5 小游戏 (opens new window)](https://cases.aotu.io/cates/游戏型/index.html)」。
...
这本小册将围绕「基础页面开发」、「响应式页面开发」、「滑屏应用开发」以及「动画效果开发」这 4 大基础能力展开叙述。「H5 游戏开发」以 [微信小游戏开发入门：从 0 到 1 实现井字棋游戏 (opens new window)](https://juejin.im/book/5b7be023e51d4538850305d0/section/5b7be024e51d45389400165b)小册独立解读，有兴趣的读者可以关注下。
...
Google 网页性能分析工具 [PageSpeed Insights (opens new window)](https://developers.google.com/speed/pagespeed/insights/)的其中一条准则就是：
...
- Mozilla[《Using the viewport meta tag to control layout on mobile browsers》(opens new window)](https://developer.mozilla.org/en-US/docs/Mozilla/Mobile/Viewport_meta_tag)
- Google[《Configure the viewport》(opens new window)](https://developers.google.com/web/fundamentals/design-and-ux/responsive/#set-the-viewport)
- Mozilla[《@viewport》(opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/@viewport)
...
我们可以从业界一些热门可靠的 CSS 框架中寻找参考答案，例如 [Bulma (opens new window)](https://bulma.io/)，其采用的「样式断点」有 5 个：
...
1. 使用桌面版的 Chrome 浏览器，打开 Google 的 [在线 Media Queries 例子 (opens new window)](https://googlesamples.github.io/web-fundamentals/fundamentals/design-and-ux/responsive/media-queries.html)直观感受下使用 Media Queries 的效果（请注意缩放浏览器窗口观察页面展示效果）
2. 了解 [Bulma (opens new window)](https://bulma.io/)框架

### 扩展阅读

- Google [《Responsive Web Design Basic》(opens new window)](https://developers.google.com/web/fundamentals/design-and-ux/responsive/)
...
体验地址：[视口单位布局 —— vw 单位(opens new window)](https://jdc.jd.com/demo/ting/vw_layout.html)
...
体验地址：[https://jdc.jd.com/demo/ting/vw_rem_layout.html(opens new window)](https://jdc.jd.com/demo/ting/vw_rem_layout.html)

### 扩展阅读

- Mozilla[《Length》(opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/length)
- 凹凸实验室 [《利用视口单位实现适配布局》(opens new window)](https://aotu.io/notes/2017/04/28/2017-4-28-CSS-viewport-units/)
...
- [Swiper (opens new window)](https://github.com/nolimits4web/Swiper)：Most modern mobile touch slider with hardware accelerated transitions.

其它更多滑屏组件的选择可查阅[《awesome-javascript》 (opens new window)](https://github.com/sorrycc/awesome-javascript#sliders)。
...
- 滑屏应用例子 [swiper.js 版本(opens new window)](http://jdc.jd.com/lab/swiper/swiper_plugin/)
- 滑屏应用例子 [hammer.js 版本(opens new window)](http://jdc.jd.com/lab/swiper/swiper_hammer/)
- 滑屏应用例子 [无依赖版本(opens new window)](http://jdc.jd.com/lab/swiper/swiper_pure/)

![img](https://user-gold-cdn.xitu.io/2018/3/6/161f909cde5960da?w=1442&h=602&f=jpeg&s=118355)
...
事实上，业界已经有许多很好用的判断手势动作的插件，如知名的 [hammer.js (opens new window)](https://hammerjs.github.io/)或 [zepto (opens new window)](http://zeptojs.com/)的 touch 模块。大家也可以通过阅读 [hammer.js (opens new window)](https://hammerjs.github.io/)的源码来进一步学习手势动作判断处理的逻辑。
...
- 滑屏应用例子源码 [Swiper 版本(opens new window)](https://coding.net/u/Jcc/p/swiper_plugin/git)
- 滑屏应用例子源码 [hammer.js 版本(opens new window)](https://coding.net/u/Jcc/p/swiper_hammer/git)
- 滑屏应用例子源码 [无依赖版本(opens new window)](https://coding.net/u/Jcc/p/swiper_pure/git)
...
*（[京东 2018 校园招聘 (opens new window)](http://wqs.jd.com/promote/201707/2018campus/index.html)）*

值得一提的是，这个滑屏 H5 案例使用了凹凸实验室自研开源的 [HTML5 构建工具 ELF (opens new window)](https://elf.aotu.io/)进行构建，利用 ELF 提供的命令行工具，我们只需敲一个命令就可以搭建一个具有基础功能的滑屏应用，大大提升开发效率。

如果说 [Swiper (opens new window)](https://github.com/nolimits4web/Swiper)是开发滑屏应用的利器，那么 [ELF (opens new window)](https://elf.aotu.io/)则是开发 HTML5 应用的利器。
...
实际上 ELF 本身基本上浓缩了前一个小节「响应式页面开发」和本小节「滑屏应用开发」的大部分内容，读者有兴趣可以移步 [ELF 官网 (opens new window)](https://elf.aotu.io/)进一步了解下 ELF 的功能特性。
...
本小节通过案例以及源码的方式，为大家介绍了如何利用业界优秀的滑屏开源组件「[Swiper (opens new window)](https://github.com/nolimits4web/Swiper)」来快速开发滑屏应用，同时也解读了滑屏应用的关键处理逻辑：手势判断处理。
...
在 CSS3 的世界里，让网页元素动起来的第一个方法是利用 `transition`，基于 `transition` 可以让元素的某个 CSS 属性从指定的开始状态过渡到特定的结束状态。我们将元素「从指定的开始状态过渡到特定的结束状态」这个过程简称为「状态变换」，注意这里的**过渡**，事实上 `transition` 便像是页面元素「状态变换」的润滑剂，如果没有 `transition`，元素「状态变换」的过程将会显得生硬而突兀（如下图中左边的小圆球，[查看 DEMO (opens new window)](https://codepen.io/mamboer/full/pLvLyv/)）。

![transition](https://user-gold-cdn.xitu.io/2018/3/10/1620f2835b9b7448?w=950&h=220&f=gif&s=97142)

`transition` 可作用于普通的 CSS 属性（如 `background` 、`opacity` ...），也可以作用于 CSS3 出现时新引入的 `transform` 属性，而利用后者可以实现 3D 的过渡效果。`transform` 属性就像是 CSS3 这个动效武器子弹里的火药，大家可以通过 [MDN (opens new window)](https://developer.mozilla.org/en-US/)的 [《transform》 (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/transform)一文进行进一步地了解学习，务必做到深谙其门道，避免一知半解。

### 一个 3D 过渡动效例子

如前面所说，利用 `transition` 结合 `transform` 可实现元素的 3D 过渡动效，所以我们这个例子的目标是：利用 `transform` 属性画一个立方体，然后利用 `transition` 实现立方体的翻转效果。大家不妨打开 [CodePen (opens new window)](https://codepen.io/)按照以下步骤亲自动手试试，或者直接 [查看 DEMO (opens new window)](https://codepen.io/mamboer/full/XEJqPy)体会最终的结果。
...
以上关键代码的关键 CSS 属性。读者也可自行在 [MDN (opens new window)](https://developer.mozilla.org/en-US/)上搜到具体的说明资料，建议结合资料和本例子亲自把玩体会。
...
通过查阅 [MDN (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Transitions/Using_CSS_transitions)资料可得 `transition` 的用法为：
...
可通过[《CSS animated properties》 (opens new window)](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_animated_properties)一文查看支持过渡动效的 CSS 属性。

1. 隐式过渡（implicit transitions）

`transition` 的过渡动画是隐式的（如下图所示，图片来源于 [MDN (opens new window)](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_Transitions/Using_CSS_transitions)），即除了动画的开始状态和结束状态我们可以自定义之外，「状态变换」的具体过程由浏览器自动执行，中途无法进行人为干预。当然，我们还可以为浏览器执行动画时指定动画的具体时长（`duration`），以及时间轴函数（`timing function`）。
...
1. 通过 Mozilla 的 [MDN (opens new window)](https://developer.mozilla.org/en-US/)文档来了解 `transition` 的详细说明及使用示例：

- [《Using CSS transitions》(opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Transitions/Using_CSS_transitions)

1. 结合 CodePen 代码示例了解 `transition` 和 `transform` 的相关属性:

- [《CSS Transitions and Transforms for Beginners》(opens new window)](https://robots.thoughtbot.com/transitions-and-transforms)
...
我们来瞅一个 `@keyframes` 的实际例子，来源于有名的 `animation` 动画库 [Animate.css (opens new window)](https://daneden.github.io/animate.css/)，其中的「[bounceIn (opens new window)](https://github.com/daneden/animate.css/blob/master/source/bouncing_entrances/bounceIn.css)」动效的关键帧代码如下：
...
- 指定元素用什么动画？（[animation-name (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-name)）
- 动画的持续时间是多少？（[animation-duration (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-duration)）
- 浏览器用什么样的时间轴函数来执行该动画？（[animation-timing-function (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-timing-function)）
- 是否需要延时执行该动画？([animation-delay (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-delay))
- 动画循环执行的次数是多少？（[animation-iteration-count (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-iteration-count)）
- 动画执行的方向是什么？（[animation-direction (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-direction)）
- 动画填充模式是什么？（[animation-fill-mode (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-fill-mode)）
- 动画执行状态是运行还是暂停？（[animation-play-state (opens new window)](https://developer.mozilla.org/en-US/docs/Web/CSS/animation-play-state)）

对于 `animation` 相关属性的介绍和使用示例，可以在 [MDN (opens new window)](https://developer.mozilla.org/en-US/)找到十分详尽的资料，这里就不搬运了，读者可以点击括号内的链接进行逐一学习。

值得一提的是，`animation` 动画的延时可以设置为负值（[试试看 (opens new window)](https://codepen.io/mamboer/pen/ZxGdpE)），善用负值的 `animation-delay` 有时候可以用最少的代码实现出乎意料的动效。

![负值animation-delay](https://user-gold-cdn.xitu.io/2018/3/12/1621a4bc7d9c9595?w=950&h=350&f=gif&s=575776)

上图的案例（[查看 DEMO (opens new window)](https://codepen.io/mamboer/full/xWGvqj)）来源于[《CSS Animation Tricks: State Jumping, Negative Delays, Animating Origin, and More》 (opens new window)](https://css-tricks.com/css-animation-tricks/)一文，利用负值 `animation-delay` 复用同一个动画轻松实现。相同的效果如果用 GIF 或 `transition` 来实现的话，恐怕会复杂很多。
...
以上两个简单的步骤完成了 `animation` 替代 `transition` 实现更丰富的动效，读者可以通过 [查看完整示例代码 (opens new window)](https://codepen.io/mamboer/pen/bvVbxv)并修改 `animation` 属性的其他设定（如 `animation-play-state` 等）来加深体会。

### 扩展阅读

1. 结合 CodePen 案例学习 `animation` 的每一个属性：[CSS Animation for Beginners(opens new window)](https://robots.thoughtbot.com/css-animation-for-beginners)
2. 学习 `animation` 动画的小奇巧：[CSS Animation Tricks: State Jumping, Negative Delays, Animating Origin, and More(opens new window)](https://css-tricks.com/css-animation-tricks/)
...
很多时候，仅仅将元素进行二维层面的变换显然不是人类的终点，毕竟十二维空间都可能不是极限（视频: [从一维空间到十二维空间 (opens new window)](http://v.youku.com/v_show/id_XNjA0MjU5NzA4.html?from=s1.8-1-1.2)）。

[Intro to 3D Transforms (opens new window)](https://desandro.github.io/3dtransforms/) 的作者 David DeSandro 说，现在可是 21 世纪，可我们竟然还在跟三十年前的二维空间界面扯皮。所幸 2011 年，我们有了 CSS3，我们还有了 3D Transforms，真是一个值得奔走相告的大事件。
...
> 视频：[透视学之一点透视法(opens new window)](https://www.bilibili.com/video/av14392523/)
...
（图片来源：[维基百科 (opens new window)](https://en.wikipedia.org/wiki/Perspective_(graphical))）
...
（图片来源：[Intro to CSS 3D transforms - Perspective (opens new window)](https://desandro.github.io/3dtransforms/docs/perspective.html)）
...
一本合上的书正常来说是在 Y 轴右侧，每一页都包含两面，也就是说一本书是由若干个翻页效果组合而成，每一页的变换原点在元素左侧。由此可以在翻牌的基础上迅速整出一个翻书 DEMO（猛戳 [查看翻书 DEMO (opens new window)](http://lyxuncle.github.io/pageturning/demo/demo.html)）。

阴影的使用能让翻书效果变得更真实。

![翻书demo](https://user-gold-cdn.xitu.io/2018/2/22/161be0897748f7f1?w=750&h=474&f=png&s=9157)

（猛戳 [查看 DEMO（带阴影） (opens new window)](http://lyxuncle.github.io/pageturning/demo/demo2.html)）
...
（图片来源：[A Guided Tour of THE MEL BIRNKRANT COLLECTION (opens new window)](http://melbirnkrant.com/collection/page48.html)）
...
（[Mozzilla 的小 DEMO (opens new window)](http://www.html5tricks.com/demo/css3-3d-book/index.html)）
...
猛戳进入 [麦芒推广页 (opens new window)](http://jdc.jd.com/fd/pp/maimang/index.html)体验 3D 立体书效果。
...
> 想象使用 GPU 加速的动画就像是 Vin Diesel（速度与激情的主角）开着 Dominic 标志性的汽车 —— Dodge Charger。它的定制 900 hp 引擎可以让它在一瞬间从 0 加速到 60 码。但是如果你开着它在拥挤的高速公路上又有什么用呢？这种情况下错的不是你的车辆，而是你还在一条拥堵的高速公路上。—— [《CSS 硬件加速的好与坏》(opens new window)](http://efe.baidu.com/blog/hardware-accelerated-css-the-nice-vs-the-naughty/)
...
- [Intro to CSS3 3D transforms (opens new window)](https://desandro.github.io/3dtransforms/) by David DeSandro —— 详尽又新鲜的 3D Transformers 手册，包含许多一看就懂的小 Demo，妈妈再也不用担心我的 3D 了。
- [Perspective (graphical) (opens new window)](https://en.wikipedia.org/wiki/Perspective_(graphical))—— 对透视学还一知半解的可以看看维基的详细说明。
- [Unfolding the Box Model: Exploring CSS 3D Transforms (opens new window)](http://rupl.github.io/unfold/) by Chris Ruppel —— 非常赞的 3D Transforms 介绍，从 2D 到 3D 过渡，启动联想学习法，一看就明白，就怕你不看。
- [CSS 硬件加速的好与坏 (opens new window)](http://efe.baidu.com/blog/hardware-accelerated-css-the-nice-vs-the-naughty/)—— 很多事情都不是一两句能讲清楚的，但是只要深入了解原理，一两句都不用讲就清楚了。
...
我们以京东 [2017 海外招聘 (opens new window)](http://jdc.jd.com/h5/jd-campus-2017/international/index.html)H5 的第三屏动画为例，为大家讲解如何利用 CSS3 实现补间动画。
...
对于 `animation-fill-mode` 属性的详解，笔者推荐[《理解 animation-fill-mode 属性》 (opens new window)](https://www.w3cplus.com/css3/understanding-css-animation-fill-mode-property.html)一文，这里不作赘述。
...
至此，整个第三屏动画就完成了，查看 [完整的 DEMO (opens new window)](http://jdc.jd.com/demo/ting/H5Demo/recruit/index.html)进行体验。
...
*读者可以自行 [下载本案例所用的 AE 稿 (opens new window)](http://jdc.jd.com/lab/2018-ae2css/apple.zip)。*
...
- [查看单个圆的补间动画(opens new window)](https://codepen.io/JChehe/full/VQOxxX)
- [完整的苹果彩蛋动画(opens new window)](https://codepen.io/JChehe/full/ddBZKY)
...
本小节以 2 个实际工作中的案例 —— [「2017 京东海外招聘 H5」 (opens new window)](http://jdc.jd.com/h5/jd-campus-2017/international/index.html)及 [「2017 年京东 App 搜索彩蛋」 (opens new window)](http://jdc.jd.com/lab/2018-ae2css/apple/)，分别为大家讲解了「如何利用 CSS3 实现补间动画」及「基于 AE 稿实现补间动画」的完整过程。在实际工作过程中，如果动画方案较为复杂，可以尝试后者，让设计师使用 AE 设计完整的动画，开发的时候基于 AE 稿来还原动效。当然，无论是哪一种方式，目前看来都免不了较大的人肉开发成本，虽然业界出现了一些能够直接将 AE 动画导出为 Web 动画的插件或开源库（代表性的有 [Bodymovin (opens new window)](https://www.adobeexchange.com/creativecloud.details.12557.html)和 [lottie-web (opens new window)](https://github.com/airbnb/lottie-web)，笔者还尝试使用 Bodymovin 实现了前面的苹果彩蛋动画，[查看 DEMO (opens new window)](http://jdc.jd.com/lab/2018-ae-apple/)），但其实际可用性还有待进一步的验证。
...
*（图片来源：[《一起翻一翻，手翻书的前世今生》 (opens new window)](http://blog.sina.com.cn/s/blog_67157b2a0102vcx3.html)）*
...
将动画帧绘制到 Canvas 上，通过不断地重绘即可实现逐帧动画。[CreateJS (opens new window)](https://www.createjs.com/)、[Pixi.js (opens new window)](http://pixijs.com/)等库都提供了成熟的方案。
...
下面以京东到家的触屏页面[《年货送到家》 (opens new window)](http://jdc.jd.com/fd/promote/201601/djnianhuo/)中的一个场景为例，为大家讲解如何利用 CSS3 来实现逐帧动画。
...
*（图片来源：[W3C (opens new window)](https://www.w3.org/TR/css3-transitions/)）*
...
前文中提到，`steps` 是 `animation-timing-function` 的一个属性值，在 [W3C (opens new window)](https://www.w3.org/TR/css3-animations/#animation-timing-function-property) 中有如下说明：
...
至此，我们便实现了一个简单的 CSS3 逐帧动画，点击[查看 DEMO (opens new window)](http://jdc.jd.com/h5/daojia-demo/)或扫描二维码：

![《年货送到家》](https://user-gold-cdn.xitu.io/2018/3/14/16224d442f9f2799?w=280&h=280&f=png&s=1534)

*[（京东年货到家逐帧动画 DEMO）(opens new window)](http://jdc.jd.com/h5/daojia-demo/)*
...
另外也可以通过 SVG 来解决抖动的问题，有兴趣可移步[《CSS 技巧：逐帧动画抖动解决方案》 (opens new window)](https://aotu.io/notes/2017/08/14/fix-sprite-anim/)做进一步的了解。

## 案例实战 2 - 利用 Canvas 做一个会动的京东 JOY

CSS3 实现的逐帧动画，如果它的帧数较多或尺寸较大时，移动端可能会存在渲染性能问题，此时建议改用 Canvas 实现。 相对于 CSS3 来说，Canvas 具有更高的学习成本，所以实际项目中推荐使用业务成熟的 Canvas 动画库，如 [CreateJS (opens new window)](https://www.createjs.com/)、[Pixi.js (opens new window)](http://pixijs.com/)等。

下面我们以 [「京东 JOY 福星会场 一一 八仙乐游记」 (opens new window)](https://h5.m.jd.com/dev/oj4d4FHfrtsP4tqT7ukC9spRX85/index.html)中的其中一个动画为例，为大家讲解如何利用 Canvas 实现逐帧动画。

![上图](https://user-gold-cdn.xitu.io/2018/3/14/162239dec61fe32a?w=320&h=320&f=gif&s=869595)

### 步骤 1 - 准备动画帧所需的雪碧图

在前面的案例中我们介绍了通过 Photoshop 手动合成雪碧图的方式，这里我们使用 [TexturePacker (opens new window)](https://www.codeandweb.com/texturepacker)工具（付费软件，帧动画开发利器，值得购买）来更快地完成同样的事情。

![TexturePacker 用户界面](https://user-gold-cdn.xitu.io/2018/3/14/16223774fc46a5c7?w=2000&h=1362&f=png&s=455019)

*（TexturePacker 用户界面）*

[TexturePacker (opens new window)](https://www.codeandweb.com/texturepacker)导出雪碧图的步骤如下：
...
[TexturePacker (opens new window)](https://www.codeandweb.com/texturepacker)除了生成雪碧图，其导出的 JSON 文件其实还包含了逐帧动画所需的运行数据，只需结合渲染载体的相应 API 即可快速实现逐帧动画。
...
这样即可完成一个逐帧动画，[点击体验 DEMO (opens new window)](https://h5.m.jd.com/dev/22Vp3Q5pX2rvuPwDYeDWjPoF3Ahf/index.html)，或扫描二维码：

![Canvas 逐帧动画 DEMO](https://user-gold-cdn.xitu.io/2018/3/14/16223a3039225360?w=280&h=280&f=png&s=2021)

*[（ Canvas 逐帧动画 DEMO ）(opens new window)](https://h5.m.jd.com/dev/22Vp3Q5pX2rvuPwDYeDWjPoF3Ahf/index.html)*

### 相关资源下载

- [逐帧动画源文件 (opens new window)](http://omwfrl50f.bkt.clouddn.com/xiaocezi/frame.zip)（用于 TexturePacker 合成）
- [TexturePacker 导出后的文件 (opens new window)](http://omwfrl50f.bkt.clouddn.com/xiaocezi/hxr.zip)（适用于未购买 TexturePacker 的读者）
- [案例 2 完整项目源码(opens new window)](https://coding.net/u/Jcc/p/canvas-sprite/git?public=true)
...
Bingo！小功告成！[查看 DEMO(opens new window)](http://jdc.jd.com/demo/simba/loading/index.html)

## 初探 SMIL

[SMIL (opens new window)](https://www.w3.org/TR/REC-smil/)的全称为 Synchronized Multimedia Integration Language（同步多媒体集成语言），按照 W3C 规范对 SMIL 的描述，它是一种允许用户在网页上定义可交互多媒体内容的 XML 语言，可结合 XHTML 和 SVG 一起使用来实现网页动态效果。
...
*（数据来源：[caniuse.com (opens new window)](https://caniuse.com/#search=SMIL)，截至 2018 年 3 月 14 日）*
...
可见 SMIL 的动画元素是可以叠加使用的，[查看 DEMO (opens new window)](https://codepen.io/mamboer/full/MVKRZJ)。

## 扩展阅读

- [Animating SVG with CSS (opens new window)](https://css-tricks.com/animating-svg-css/)：利用 SVG 结合 CSS 实现一个动态广告图
- [Creating Cel Animations With SVG (opens new window)](https://www.smashingmagazine.com/2015/09/creating-cel-animations-with-svg/)：利用 SVG 结合 CSS 实现逐帧（定格）动画
- [Cel Animation (opens new window)](https://github.com/Heydon/cel-animation)：用于辅助实现 SVG 逐帧（定格）动画的一个 SASS @mixin 函数
- [SVG - animationMotion (opens new window)](https://developer.mozilla.org/en-US/docs/Web/SVG/Element/animateMotion)：了解 `animationMotion` 元素
- [SVG animation with SMIL (opens new window)](https://developer.mozilla.org/en-US/docs/Web/SVG/SVG_animation_with_SMIL)：了解使用 SMIL 实现 SVG 动画的方法

## 小结

SVG 在网页中的角色更像是类似图片一样的媒体对象，其动画也常常和 CSS 有关联，当然利用 [SMIL (opens new window)](https://developer.mozilla.org/en-US/docs/Web/SVG/SVG_animation_with_SMIL)仍然可以为 SVG 添加独立的动效，除了微软系的浏览器不支持之外，其兼容性还是相当可观的。在平时工作过程中，矢量图标的动画、轨迹动画特别适合使用 SVG 来实现。
...
根据 [维基 (opens new window)](http://zh.wikipedia.org/wiki/动画)的释义，动画是指由许多帧静止的画面，以一定的速度（如每秒 16 张）连续播放时，肉眼因视觉残象产生错觉，而误以为画面活动的作品（GIF 图片正是运用这种原理）。因此最初的动画是通过几张快速翻动的连续画面制作而成，而后经历了电影摄影技术的出现、电脑科技的进步，逐渐转向数字化。
...
在[《动画制作流程介绍》 (opens new window)](http://cghappening.blogspot.com/2011/02/blog-post_23.html)提供的视频中可以看到关键帧在动画制作中所起的地基般的作用。
...
[[窃听风云3\]预告片 (opens new window)](http://movie.douban.com/trailer/156598/#content)中字幕出现的方式表现出信号干扰的效果，由此可以将影片相关专题作出这样的开场动画——

![窃听风云3](https://user-gold-cdn.xitu.io/2018/3/5/161f55894e8dac08?w=520&h=317&f=gif&s=2894038)

简单的几个不同色调的图片进行替换就能做出类似效果，代码也灰常简单（看 [demo (opens new window)](http://jdc.jd.com/demo/ec/movieeye/doing_movie_by_css3.html)）：
...
这时需要注意的是因果关系对动画的影响，“一个动画师必须懂得自然界物体运动的力学知识”，这样“才能创造情绪和表达正确的感觉。” 我们来看看为了使动画更加流畅真实，迪士尼爷爷想出了什么办法。「白雪公主与七个小矮人」作为 80、90 后动画电影启蒙，使用了一项革新动画制作的技术——[转描机 (opens new window)](http://zh.wikipedia.org/wiki/转描机技术)。

![转描机](https://user-gold-cdn.xitu.io/2018/3/5/161f5799d8dee2b0?w=400&h=280&f=gif&s=7797119)

*（图片来源：视频 [【DizAvenue】制作白雪公主的故事 The Making of Snow White (opens new window)](https://www.bilibili.com/video/av19937746/)）*

#### 迪士尼动画十二原则的运用

视频中有一个细节，迪士尼爷爷让动画师注意那位大叔在跳踢踏舞时重力对裤腿的作用（20 分 20 秒）。是的，迪士尼爷爷强调的就是动画与物理学的关系。

迪士尼工作室在早期，将他们在动画实践中所总结出的规律总结成为了经典的[《动画12原则》 (opens new window)](https://zh.wikipedia.org/wiki/動畫的12項基本法則)，可以说这 12 原则就是让迪士尼动画在当时脱颖而出的黄金原则，即便是现如今看来，这 12 原则也并不过时，甚至可以扩展运用至网页交互动效中。
...
*（dribbble’s stripe checkout，图片来源：[The Art of Animation (opens new window)](http://markgeyer.com/pres/the-art-of-ui-animations/#/2/9)）*

![Giving Animations Life](https://user-gold-cdn.xitu.io/2018/3/5/161f57b9c771658c?w=396&h=534&f=gif&s=1328667)

*（图片来源：[Giving Animations Life (opens new window)](https://medium.com/tictail-makers/giving-animations-life-8b20165224c5)）*

在实现过程中，我们可以选择合适的时间轴函数来达成这些效果，或者使用 [Bounce.js (opens new window)](http://bouncejs.com/)这样的弹性动效库，给元素加上弹性动效后，通常能够事半功倍地增加动效的活力与吸引力，但同样的，过犹不及，一切动效的添加都要讲究适度。

对于时间轴函数之贝塞尔曲线的应用，笔者推荐两个工具：[贝塞尔曲线在线调试工具 (opens new window)](http://cubic-bezier.com/#.17,.67,.83,.67)以及 [贝塞尔缓动函数库 (opens new window)](http://easings.net/zh-cn#)，能够帮助大家快速选取合适的贝塞尔曲线函数。
...
对于这十二条原则的进一步学习，笔者推荐[《入门必读！迪士尼影响至今的十二条动画经典法则 》 (opens new window)](http://www.zcool.com.cn/article/ZNDI1ODAw.html)一文，里面介绍了每条法则的具体内容，必定能让你有所受益。

#### 对动画过渡时间的把握

Google 在 Material Design 指南中提及 [动效过渡时间 (opens new window)](https://material.io/guidelines/motion/duration-easing.html#duration-easing-speed)：对于用于交互操作场景的补间动画而言，短动画时间应该控制在 150～200ms，而长动画控制在 300～400ms。而对于用于呈现场景的补间动画则视具体而定，以真实世界为参考标准是最好的。
...
*（动画来源：[Pseudo-Elements Animations and Transitions (opens new window)](http://tympanus.net/Development/PseudoElementsAnimationsTransitions/index4.html)）*
...
笔者这里推荐 [CSS Triggers (opens new window)](http://csstriggers.com/)，通过它你可以查阅到各 CSS 属性及其影响的环节，从而避免不小心使用到性能开销较大的属性。

除此之外，对动画渲染的优化还有其他方式，这里抛出 @登平登平 在 [《H5 动画 60fps 之路》 (opens new window)](https://weibo.com/p/1001603865643593165786)一文中的总结，同学们可以前往原文进行进一步的学习。

![H5 动画 60fps 之路](https://user-gold-cdn.xitu.io/2018/3/5/161f66575fbbb137?w=1632&h=628&f=png&s=375991)

## 扩展阅读

- [《入门必读！迪士尼影响至今的十二条动画经典法则 》 (opens new window)](http://www.zcool.com.cn/article/ZNDI1ODAw.html)—— 详细了解迪士尼十二条动画经典法则
- [《让界面动画更自然－ISUX》 (opens new window)](http://blog.jobbole.com/76194/)—— 如何合理选用动画曲线函数
- [贝塞尔曲线在线调试工具 (opens new window)](http://cubic-bezier.com/#.17,.67,.83,.67)以及 [贝塞尔缓动函数库 (opens new window)](http://easings.net/zh-cn#)—— 帮助你快速选取合适的贝塞尔曲线函数
- [The Guide To CSS Animation: Principles and Examples (opens new window)](https://www.smashingmagazine.com/2011/09/the-guide-to-css-animation-principles-and-examples/)—— 很老的一篇英文文章，以示例的方式介绍了迪士尼的十二条动画法则
